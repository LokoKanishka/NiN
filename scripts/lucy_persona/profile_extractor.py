...
No inventes datos que no estén en los ejemplos. Si no hay ejemplos suficientes en un rubro, índicalo.
...
def extract_profile(max_examples=15):
    ...
    if not os.path.exists(SCORED_EXAMPLES_PATH):
        print(f"❌ Error: {SCORED_EXAMPLES_PATH} not found. Run scorer first.")
        # Create a stub if the user runs the whole pipe without real data
        with open(PROFILE_PATH, "w") as f:
            f.write("# Perfil de Identidad: Lucy\n\n*(Generación pendiente: No hay suficientes datos procesados)*")
        return
    ...
    if not scored_items:
        print("⚠️ No 'accepted' items found in scored_examples. Skipping profile extraction.")
        with open(PROFILE_PATH, "w") as f:
            f.write("# Perfil de Identidad: Lucy\n\n*(Generación pendiente: No se hallaron ejemplos aceptados)*")
        return
    ...
    print(f"🧠 Synthesizing persona profile from the top {len(top_items)} empirical examples...")
